...
public class ServerConfig {
    ...
    public ServerConfig(String id, ServerType type, Path workdir, String jar,
                        ...
                        boolean watchdogEnabled, int maxRestarts, int cooldown) {
        ...
        this.watchdogMaxRestarts = maxRestarts;
        this.watchdogCooldown = cooldown;
    }
    ...
    public static ServerConfig load(String id, ServerType type, Path workdir,
        ...
        int     maxR       = Integer.parseInt(p.getProperty("watchdog_max_restarts", "5"));
        int     cooldown   = Integer.parseInt(p.getProperty("watchdog_cooldown", "10"));
        return new ServerConfig(id, type, workdir, jar, heap, extraFlags, port, watchdog, maxR, cooldown);
    }
    ...
    public List<String> buildJvmArgs() {
        ...
        for (String f : type.defaultFlags().split("\\s+")) if (!f.isBlank()) args.add(f);
        args.add("-Xmx" + heap);
        if (!extraFlags.isBlank())
            for (String f : extraFlags.strip().split("\\s+")) if (!f.isBlank()) args.add(f);
